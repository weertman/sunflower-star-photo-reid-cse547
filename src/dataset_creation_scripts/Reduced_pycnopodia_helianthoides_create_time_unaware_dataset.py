...
def reduced_pycnopodia_helianthoides():
    create_n_datasets = 3

    accepted_groups = ['wild_stars_cama_beach_pycno_pre_deployment',
                       'PWS_2023',
                       'lab_stars_pycno_deployment',
                       'HodinLab_Y_Tank_01_30_2023']
    ...
    for n in range(create_n_datasets):
        ...
        dst_data_dir_head = f'Reduced_{target_class}_full_'

        src_data_root = r'C:\Users\wlwee\OneDrive\Desktop\sorted_stars'
        tmp_src_image_files = glob.glob(os.path.join(src_data_root, '*', '*', '*', '*.*'))

        src_image_files = []
        for group in accepted_groups:
            src_image_files += [x for x in tmp_src_image_files if group in x]
