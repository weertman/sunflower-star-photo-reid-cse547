...
def process_image_file (image_file, target_class, model, yolo_classes, conf=0.25):
    ...
    if len(indexes) > 0:
        ...
        box = boxes[index].cpu().numpy()[0]
        ## crop the raw image using the box
        x, y, _, _ = map(int, box)
        _, _, w, h = map(int, box/2)
        mask_image = image[y-h:y+h, x-w:x+w]
...
def process_df (df, df_dir, target_class, model, yolo_classes, conf=0.25,
    ...
    for _, row in df.iterrows():
        ...
        mask_image = process_image_file(image_file, target_class, model, yolo_classes, conf=conf)
...
def other_species():
    ...
    for n in range(create_n_datasets):
        ...
        target_class = 'Other_species'

        dst_data_dir_head = f'Unmasked_{target_class}_sea-star-re-id-full_'

        src_data_root = r'E:\SunflowerStarArchive\triplet-loss-animal-reid-main\experiments'
        ...
        for image_file in src_image_files[:5]:
            print(image_file)
            id_code = os.path.basename(os.path.dirname(image_file))
        ...
        df = pd.DataFrame(
            {'id_code': id_codes, 'image_file': src_image_files})
        print(df.head())
        print(df.shape)
